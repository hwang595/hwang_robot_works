...
def do_calib(moCap_time_stamp, pos_moCap, time_stamp_rob, pos_rob, test_plot=False):
...
#	moCap_time_stamp, moCap_pos_x, moCap_pos_y, moCap_pos_z = parsePosTables(rigidBodyPos)
	moCap_pos_x, moCap_pos_y, moCap_pos_z = parsePosTables(pos_moCap)
	pos_rob_x, pos_rob_y, pos_rob_z = parsePosTables(pos_rob)

	start_stamp = max(moCap_time_stamp[0], time_stamp_rob[0])
	end_stamp = min(moCap_time_stamp[-1], time_stamp_rob[-1])
...
def parse_rog_bag(ros_bag):
	t_robot = []
	t_tongs = []
	pos_robot = []
	pos_tongs = []
	for topic, msg, t in bag.read_messages(topics=['/Robot_3/pose', '/gripper/center_position']):
		if topic == '/Robot_3/pose':
			t_tongs.append(t)
			pos_tongs.append([msg.pose.position.x, msg.pose.position.y, msg.pose.position.z])
		elif topic == '/gripper/center_position':
			t_robot.append(t)
			pos_robot.append([msg.pose.position.x, msg.pose.position.y, msg.pose.position.z])
	return t_robot, pos_robot, t_tongs, pos_tongs
...
#	rigidBodyPos, rigidBodyQuat = bag_data.parseTongsBag(bagname)
	...
	t_robot, pos_robot, t_tongs, pos_tongs = parse_rog_bag(bag)
	#load robot position data and parse it to get robot position matrix
#	fileName = "calib_robot_log/calibration_new_round.txt"
#	time_stamp_rob, pos_rob, joint_angles = readRobotReaderFile(fileName)

	pos_rob_x, pos_rob_y, pos_rob_z = parsePosTables(pos_robot)
	#reshape the joint angles to make it easier to interpolate
#	reshaped_joint_angles = np.transpose(np.array(joint_angles))
#	moCap_time_stamp, moCap_pos_x, moCap_pos_y, moCap_pos_z = parsePosTables(rigidBodyPos)
	matrix_after_transform, M_mocap_robot = do_calib(
		moCap_time_stamp=t_tongs, 
		pos_moCap=pos_tongs, 
		time_stamp_rob=t_robot, 
		pos_rob=pos_robot, 
		test_plot=False)
#	rob_EE_pos_x, rob_EE_pos_y, rob_EE_pos_z = parsePosTables(rob_endEffector_pos)
...
#	plt.plot([i for i in range(len(CC_x))], CC_x, c='--r')
	plt.plot([i for i in range(len(pos_rob_x))], pos_rob_y, c='g')
#	plt.plot([i for i in range(len(CC_y))], CC_y, c='--g')
	plt.plot([i for i in range(len(pos_rob_x))], pos_rob_z, c='b')
#	plt.plot([i for i in range(len(CC_z))], CC_z, c='--b')
	plt.show()
