# -*- coding: utf-8 -*-
"""
Created on Sun Mar 11 10:47:13 2012

@author: gleicher
"""

## {{{ http://code.activestate.com/recipes/491268/ (r2)
...
def rank(x, NoneIsLast=True, decreasing = False, ties = "first", frank=False):
    """
    ...
    Ties are NOT averaged by default. Choices are:
        ...
        hacked by gleicher - add "frank" - if this is not "False" then assume
        we have floating point numbers, and use as a tolerance
    """
    omitNone = False
    if NoneIsLast == None:
        NoneIsLast = True
    ...
    newblock   = []
    for i in range(1,n) :
        # added by gleicher, 3/25/2015
        equal = abs(x[O[i]]-x[O[i-1]])<frank if frank else x[O[i]] == x[O[i-1]]
        if equal:
