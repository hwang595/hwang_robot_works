# -*- coding: utf-8 -*-
"""
Created on Thu Sep 23 15:38:54 2010
...
# keep everything in these "global" (to the module) variables
schemeNames = []
schemeTypes = []
schemes = dict()
schemeTypesNames = dict()
...
# unfortunately, we may not be able to find the file (since the path)
# might be weird - so we need to just put it here in place 
cbtext = """Accent,3,qual,,1,A,127,201,127,Qualitative,,,,,,
,,,,2,B,190,174,212,,,,,,,
,,,,3,C,253,192,134,,,,,,,
...
# read in the file and build the table
def readCB():
    print "Reading ColorBrewer file"
    # the "current" thing we are reading
    schid = None
    colors = []
    
    # now go through the lines
    f1 = cbtext.splitlines()
    for li in f1:
        words = li.split(',')
        # see if this is the first line of a scheme 
        if len(words[0]) > 0:
            # if we were working on a scheme, add it
            if schid:
                schemes[schid] = colors
                schid = None
            if not(words[0] in schemeNames):
                schemeNames.append(words[0])
            if not(words[2] in schemeTypes):
                schemeTypes.append(words[2])
            if words[2] not in schemeTypesNames:
                schemeTypesNames[words[2]] = set()
            schemeTypesNames[words[2]].add(words[0])
            schid = (words[0],int(words[1]),words[2])
            colors = []
        colors.append((toRGBi(words[6]),toRGBi(words[7]),toRGBi(words[8])))
    # be sure to add the last of the schemes
    if schid:
        schemes[schid] = colors
        schid = None
...
def hex_to_rgb(value):
    value = value.lstrip('#')
    lv = len(value)
    sf = 16 if lv==3 else 1
    trip = tuple(int(value[i:i+lv/3], 16)*sf for i in range(0, lv, lv/3))
    return trip
    
def dimRGB((r,g,b),val=.7):
    return (int(float(r)*val), int(float(g)*val), int(float(b)*val))
    
def getScheme(name="YlOrRd", size=9, type="seq"):
    return schemes[(name,size,type)]
def getHexScheme(name="YlOrRd", size=9, type="seq"):
    rgb = schemes[(name,size,type)]
    return map(lambda x: rgb_to_hex(x), rgb)
    
def lerp(u,a,b):
    u1 = 1-u
    return ( int(u1*float(a[0])+u*float(b[0])),
             int(u1*float(a[1])+u*float(b[1])),
             int(u1*float(a[2])+u*float(b[2])) )
    
def lerpScheme(pct,scheme):
    if (pct>1):
        pct = 1
    if (pct<0):
        pct = 0
    ...
    if (fp<.01):
        ...
        return lerp(fp,scheme[ipi],scheme[ipi+1])
  
# tries to make an intelligent choice on what colors to use
# based on set size
# will do interpolation if you have more than 9 items
# will sort things to make them sequential
def setToColor(labels, scheme=None, stype=None, tohex=False):
    nc = len(labels)
    cs = dict()
    ...
    ssize = nc if nc<9 else 7
    
    colors = getScheme(name=sname,size=ssize,type=stype)
    # if (nc <= 7):
    ...
    sm = float(len(sl)-1)
    for lab in labels:
        c = lerpScheme(float(sl.index(lab))/sm,colors)
        cs[lab] = rgb_to_hex(c) if tohex else c
