...
if __name__ == '__main__':
    rospy.init_node('tmp_rb_center')

    listener = tf.TransformListener()

    gripper_center_pub = rospy.Publisher('tmp_rb_center', PoseStamped, queue_size=10)

    rate = rospy.Rate(100.0)
    while not rospy.is_shutdown():
        try:
            (trans_center, rot_center) = listener.lookupTransform('ur5/base_link', 'Robot_3/base_link',
							      rospy.Time())
