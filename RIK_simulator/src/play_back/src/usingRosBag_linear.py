...
class RosBagParser:
    ...
    def quart_slerp_interpolate(self, quart_w, quart_x, quart_y, quart_z, time_stamp, global_end_time):
        ...
        for t_idx in range(len(time_interval_obj)):
            time_comp = round(float(time_stamp[range_counter].to_nsec() - time_stamp[0].to_nsec()) / 1e9, 3)
            last_time_comp = round(float(time_stamp[range_counter-1].to_nsec() - time_stamp[0].to_nsec()) / 1e9, 3)
    ...
    def detect_if_emtpy(sefl, encoder_array, force1_array, force2_array, viveQuat_array, vivePos_array):
        empty_list = [0] * 5
        if len(encoder_array) == 0:
            empty_list[0] = 1
        if len(force1_array) == 0:
            empty_list[1] = 1
        if len(force2_array) == 0:
            empty_list[2] = 1
        if len(viveQuat_array) == 0:
            empty_list[3] = 1
        if len(vivePos_array) == 0:
            empty_list[4] = 1
        return empty_list

    def calculate_global_end_time(
        self, empty_flag_list, encoderarray, forceSensor1array, forceSensor2array, time_quat, time_pos):
        end_time_list = []
        if empty_flag_list[0] == 0:
            encoder_end_time = float(encoderarray[-1][0].to_nsec() - encoderarray[0][0].to_nsec()) / 1e9
        ...
        if empty_flag_list[1] == 0:
            force1_end_time = float(forceSensor1array[-1][0].to_nsec() - forceSensor1array[0][0].to_nsec()) / 1e9
        ...
        if empty_flag_list[2] == 0:
            force2_end_time = float(forceSensor2array[-1][0].to_nsec() - forceSensor2array[0][0].to_nsec()) / 1e9
        ...
        if empty_flag_list[3] == 0:
            quat_end_time = float(time_quat[-1].to_nsec() - time_quat[0].to_nsec()) / 1e9
        ...
        if empty_flag_list[4] == 0:
            pos_end_time = float(time_pos[-1].to_nsec() - time_pos[0].to_nsec()) / 1e9
    ...
    def  parseTongsBag(self, bagname):
        encoder_is_empty = 0 #we make several flags here to make sure if one or several channels of signal are empty
        force1_is_empty = 0
        force2_is_empty = 0
        viveQuat_is_empty = 0
        vivePos_is_empty = 0
        empty_flag_list = [encoder_is_empty, force1_is_empty, force2_is_empty,
                            viveQuat_is_empty, vivePos_is_empty]
        ...
        empty_flag_list = self.detect_if_emtpy(
        ...
        global_end_time = self.calculate_global_end_time(
            ...
            forceSensor2array, time_quat, time_pos)
        ...
        self.timeStampArray = np.array(self.resample_time_stamp) # added by DR to easily binary search
...
if __name__ == "__main__":
    ...
    empty_flag_list = bag_data.parseTongsBag(bagname)
    print bag_data.original_sampleRate
